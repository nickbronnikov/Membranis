        ...
        switch ($file_info['extension']) {
            case 'pdf':
                ...
                if (move_uploaded_file($_FILES['upl']['tmp_name'], 'users_files/' . $data[0]['folder'] . '/' . $cryptname . '/' . $name)) {
                    ...
                    B::inBase('users_files', array('id_user', 'path', 'original_name', 'author', 'cover'), array($data[0]['id'], 'users_files/' . $data[0]['folder'] . '/' . $cryptname . '/' . $name, $file_info['filename'], $file_info['filename'], 'users_files/' . $data[0]['folder'] . '/' . $cryptname . '/' . 'cover.jpg'));
                }
                break;
            case 'fb2':
